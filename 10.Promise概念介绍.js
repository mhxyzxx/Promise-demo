...
// 由以下代码可得一个结论：每当 new 一个 Promise 实例的时候，就会立即 执行这个 异步操作中的代码
...
// 以上代码只要new 一个 Promise 实例，里面的异步代码就立即执行了，但这并不是我想要的效果，我想只要在调用的时候才让它执行。
// 此时，我们应该想到function方法。因为只要function中的代码只要调用才会执行的。


// 初衷： 给路径，返回读取到的内容
// 分析下面代码执行顺序：当我们函数getFileByPath一调用，它就会立即执行Promise对象，一瞬间的事，然后就执行.then的方法，此时
// 因我们的读文件方法fs.readFile是异步函数，还没执行，等到then方法执行完了，两个成功和失败的回调函数已初始化完毕。等到异步函数
// 成功读取文件，把结果传给两个回调函数并调用回调函数。在then方法中也得到了结果。
